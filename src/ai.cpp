...
int balanceVerdict(Sprite* sprite, int id) {
    ...
    int ret = 0;
    auto* player = static_cast<Sprite *>(spriteSnake[id]->sprites->head->element);
...
int testOneMove(Snake* snake, Direction direction) {
    auto* snakeHead = static_cast<Sprite *>(snake->sprites->head->element);
    ...
    if (change) {
        ...
        for (int i = LEFT; i <= DOWN; i++)
            if (i != currentDirection && (i ^ 1) != currentDirection) {
                int value = testOneMove(snake, static_cast<Direction>(i));
                if (value >= originValue) choices[count++] = (Choice){value, static_cast<Direction>(i)};
            }
        if (count) {
