...
Score** startGame(int localPlayers, int remotePlayers, bool localFirst) {
    auto** scores = static_cast<Score **>(malloc(sizeof(Score *) * localPlayers));
...
void appendSpriteToSnake(
    ...
    if (snake->sprites->head) {
        snakeHead = static_cast<Sprite *>(snake->sprites->head->element);
...
void initPlayer(int playerType) {
    ...
    Snake* p = spriteSnake[playersCount] =
                       createSnake(MOVE_STEP, playersCount, static_cast<PlayerType>(playerType));
...
void freezeSnake(Snake* snake, int duration) {
    ...
    Effect* effect = NULL;
    if (snake->buffs[BUFF_DEFFENCE]) {
        effect = static_cast<Effect *>(malloc(sizeof(Effect)));
...
void shieldSnake(Snake* snake, int duration) {
    ...
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
        shieldSprite(static_cast<Sprite *>(p->element), duration);
    }
}
...
void attackUpSnkae(Snake* snake, int duration) {
    ...
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
        attackUpSprite(static_cast<Sprite *>(p->element), duration);
    }
}
...
bool crushVerdict(Sprite* sprite, bool loose, bool useAnimationBox) {
    ...
    for (int i = 0; i < spritesCount; i++) {
        ...
        for (LinkNode* p = spriteSnake[i]->sprites->head; p; p = p->nxt) {
            auto* other = static_cast<Sprite *>(p->element);
            if (other != sprite) {
...
bool makeSnakeCross(Snake* snake) {
    ...
    for (int i = 0; i < SCREEN_WIDTH / UNIT; i++)
        for (int j = 0; j < SCREEN_HEIGHT / UNIT; j++)
            if (hasMap[i][j]) {
                ...
                if (isPlayer(snake)) {
                    SDL_Rect headBox = getSpriteFeetBox(static_cast<Sprite *>(snake->sprites->head->element));
    ...
    for (LinkNode *p = snake->sprites->head, *nxt; p; p = nxt) {
        ...
        if (sprite->hp <= 0) {
            for (LinkNode* q = snake->sprites->tail; q != p; q = q->pre) {
                auto *prevSprite = static_cast<Sprite *>(q->pre->element), *sprite = static_cast<Sprite *>(q->element);
    ...
    if (!snake->sprites->head) return false;
    Sprite* snakeHead = static_cast<Sprite *>(snake->sprites->head->element);
    bool die = crushVerdict(snakeHead, !isPlayer(snake), false);
    if (die) {
        for (LinkNode* p = snake->sprites->head; p; p = p->nxt) {
            Sprite* sprite = static_cast<Sprite *>(p->element);
            sprite->hp = 0;
        }
    }
...
bool makeBulletCross(Bullet* bullet) {
    ...
    if (!hasMap[bullet->x / UNIT][bullet->y / UNIT]) {
        Animation* ani = static_cast<Animation *>(malloc(sizeof(Animation)));
    ...
    if (!hit)
        for (int i = 0; i < spritesCount; i++)
            if (bullet->team != spriteSnake[i]->team) {
                for (LinkNode* p = spriteSnake[i]->sprites->head; p; p = p->nxt) {
                    Sprite* target = static_cast<Sprite *>(p->element);
...
void makeCross() {
    ...
    for (LinkNode *p = bullets->head, *nxt; p; p = nxt) {
        nxt = p->nxt;
        if (makeBulletCross(static_cast<Bullet *>(p->element))) {
            auto* bullet = static_cast<Bullet *>(p->element);
...
void makeSpriteAttack(Sprite* sprite, Snake* snake) {
    ...
    for (int i = 0; i < spritesCount; i++)
        if (snake->team != spriteSnake[i]->team) {
            for (LinkNode* p = spriteSnake[i]->sprites->head; p; p = p->nxt) {
                auto* target = static_cast<Sprite *>(p->element);
...
void makeSnakeAttack(Snake* snake) {
    if (snake->buffs[BUFF_FROZEN]) return;
    for (LinkNode* p = snake->sprites->head; p; p = p->nxt)
        makeSpriteAttack(static_cast<Sprite *>(p->element), snake);
}
bool isWin() {
                ...
                if (player->playerType == LOCAL) {
                    if (!player->buffs[BUFF_FROZEN] && player->sprites->head != NULL) {
                        ...
                        if (direction >= 0) {
                            ...
                            changeSpriteDirection(player->sprites->head, static_cast<Direction>(direction));
                        }
                    }
                }
            }
