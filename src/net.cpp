...
void hostGame() {
    ...
    while (!lanClientSocket) {
        SDLNet_TCP_Accept(lanServerSocket);
        clearRenderer();
    ...
    for (auto & i : listening) i->destroyText();
...
void joinGame(const char* hostname, Uint16 port) {
    ...
    while (!lanClientSocket) {
        ...
        SDLNet_TCP_Open(&serverIp);
        if (lanClientSocket == nullptr) {
    ...
    for (auto & i : connecting) i->destroyText();
